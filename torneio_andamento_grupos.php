<!--STANDING CUP BEGIN-->
<section class="standing-cup">
    <div class="container">
    <h2>Fase de grupos</h2>

        <div class="row">


        <ul class="nav tab-filters">

            <li class=""><a data-toggle="tab" href="#3divisao">3ª Divisão - Grupo A</a></li>
            <li class=""><a data-toggle="tab" href="#3divisaob">3ª Divisão - Grupo B</a></li>
            <li class=""><a data-toggle="tab" href="#2divisaoa">2ª Divisão - Grupo A</a></li>
            <li class=""><a data-toggle="tab" href="#2divisaob">2ª Divisão - Grupo B</a></li>
            <li class=""><a data-toggle="tab" href="#1divisao">1ª Divisão</a></li>
            <li class="active"><a data-toggle="tab" href="#ranking">Ranking Geral</a></li>

        </ul>


        <style>
       
       /**/
        </style>

        <div class="tab-content">

<!--3 Divisão Grupo A1 --------------->
<div id="3divisao" class="tab-pane fade">
        <table>
                <tr>
                <th colspan="2" width="20%" style=" background-color:#FFF200; color:black">Classificado para 2ª Divisão Grupo A</th>
                </tr>
            </table>
    <section class="main-match-shedule">
            <div class="row">
            <div class="col-md-6">
                <h6>Ranking</h6>
                <div class="club-standings overflow-scroll">
                    <table class="table-standings">
                        <tr>
                            <th>Jogador</th>
                            <th>Vitória</th>
                            <th>Estrela</th>
                            <th>Porcentagem</th>
                        </tr>

                        <!-- 1º Lugar-->
                        <tr class="teste">
                    <!-- Nick Vila --><td style="background-color: #FFF200 !important;"><b>1</b> <span class="team"></span> <span>DANILO</span></td>
                    <!-- Vitorias --> <td style="background-color: #FFF200 !important;">6</td>
                    <!-- Estrelas --> <td style="background-color: #FFF200 !important;">15</td>
                    <!-- Porcentagem--><td style="background-color: #FFF200 !important;">85,71%</td>
                        </tr>

                        <!-- 2º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>2</b> <span class="team"></span> <span>CTB MELCHORL93</span></td>
                    <!-- Vitorias --> <td>5</td>
                    <!-- Estrelas --> <td>15</td>
                    <!-- Porcentagem--><td>90.71%</td>
                        </tr>

                        
                        <!-- 3º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>3</b> <span class="team"></span> <span>BLANC-TUGA</span></td>
                    <!-- Vitorias --> <td>5</td>
                    <!-- Estrelas --> <td>14</td>
                    <!-- Porcentagem--><td>83,57%</td>
                        </tr>

                        <!-- 4º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>4</b> <span class="team"></span> <span>BRUNOSLB190</span></td>
                    <!-- Vitorias --> <td>4</td>
                    <!-- Estrelas --> <td>14</td>
                    <!-- Porcentagem--><td>80.71%</td>
                        </tr>

                    <!-- 5º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>5</b> <span class="team"></span> <span>CAETANO RCK</span></td>
                    <!-- Vitorias --> <td>3</td>
                    <!-- Estrelas --> <td>15</td>
                    <!-- Porcentagem--><td>85,43%</td>
                    </tr>

                    <!-- 6º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>6</b> <span class="team"></span> <span>SILVIO</span></td>
                    <!-- Vitorias --> <td>3</td>
                    <!-- Estrelas --> <td>14</td>
                    <!-- Porcentagem--><td>75,71%</td>
                    </tr>

                    <!-- 7º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>7</b> <span class="team"></span> <span>🔶️IE-LEONARDO🔶️</span></td>
                    <!-- Vitorias --> <td>1</td>
                    <!-- Estrelas --> <td>13</td>
                    <!-- Porcentagem--><td>66,29%</td>
                    </tr>

                    <!-- 8º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>8</b> <span class="team"></span> <span>PEDRO FHELIPE</span></td>
                    <!-- Vitorias --> <td>1</td>
                    <!-- Estrelas --> <td>11</td>
                    <!-- Porcentagem--><td>60,71%</td>
                    </tr>


                    </table>
                </div>
            </div>
            
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Primeira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CTB MELCHORL93 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>97% : 45%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i>&emsp;PEDRO FHELIPE </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>🔶️IE-LEONARDO🔶️ &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>93% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;CAETANO RCK </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>BLANC-TUGA &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>80% : 77%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;DANILO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>BRUNOSLB190 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>85% : 73%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;SILVIO </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Segunda rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CTB MELCHORL93 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 97%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;CAETANO RCK </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>DANILO &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>87% : 49%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i>&emsp;PEDRO FHELIPE</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>🔶️IE-LEONARDO🔶️ &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>59% : 88%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;SILVIO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>BLANC-TUGA &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>93% : 75%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;BRUNOSLB190 </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Terceira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CTB MELCHORL93 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>78% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;DANILO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CAETANO RCK &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>82% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;SILVIO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>PEDRO FHELIPE &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>62% : 72%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;BRUNOSLB190</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>🔶️IE-LEONARDO🔶️ &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>70% : 65%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i>&emsp;BLANC-TUGA </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quarta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CTB MELCHORL93 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>94% : 68%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;SILVIO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>DANILO &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>90% : 18%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;BRUNOSLB190</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>CAETANO RCK &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>77% : 78%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;BLANC-TUGA</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>PEDRO FHELIPE &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>92% : 56%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i>&emsp;🔶️IE-LEONARDO🔶️ </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quinta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CTB MELCHORL93 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>83% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;BRUNOSLB190</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>SILVIO &emsp;<i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>72% : 83%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;BLANC-TUGA</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>DANILO &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>84% : 56%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i>&emsp;🔶️IE-LEONARDO🔶️</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>CAETANO RCK &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>95% : 76%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;PEDRO FHELIPE </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sexta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CTB MELCHORL93 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>89% : 86%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> &emsp;BLANC-TUGA</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>BRUNOSLB190 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>73% : 68%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;🔶️IE-LEONARDO🔶️</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>SILVIO &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>74% : 43%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i>&emsp;PEDRO FHELIPE</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>DANILO &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>90% : 77%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;CAETANO RCK</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sétima rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CTB MELCHORL93 &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>94% : 62%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> &emsp;🔶️IE-LEONARDO🔶️</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>BLANC-TUGA &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 58%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;PEDRO FHELIPE</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>BRUNOSLB190 &emsp;<i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>79% : 70%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;CAETANO RCK</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>SILVIO &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>55% : 72%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;DANILO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

            </div>
    </section>

                </div>

<!--3 Divisão Grupo B --------------->
<div id="3divisaob" class="tab-pane fade">
            <table>
                    <tr>
                    <th colspan="2" width="20%" style=" background-color:#E1B12C; color:black">Classificado para 2ª Divisão Grupo B</th>
                    </tr>
             </table>
    <section class="main-match-shedule">
            <div class="row">
            <div class="col-md-6">
                <h6>Rankindedeg</h6>
                <div class="club-standings overflow-scroll">
                    <table class="table-standings">
                        <tr>
                            <th>Jogador</th>
                            <th>Vitória</th>
                            <th>Estrela</th>
                            <th>Porcentagem</th>
                        </tr>

                        <!-- 1º Lugar-->
                        <tr class="teste">
                    <!-- Nick Vila --><td style="background-color: #FFF200 !important;"><b>1</b> <span class="team"></span> <span>YONDAIME</span></td>
                    <!-- Vitorias --> <td style="background-color: #FFF200 !important;">3</td>
                    <!-- Estrelas --> <td style="background-color: #FFF200 !important;">9</td>
                    <!-- Porcentagem--><td style="background-color: #FFF200 !important;">89%</td>
                        </tr>

                        <!-- 2º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>2</b> <span class="team"></span> <span>JOAOZIN</span></td>
                    <!-- Vitorias --> <td>3</td>
                    <!-- Estrelas --> <td>8</td>
                    <!-- Porcentagem--><td>85,75%</td>
                        </tr>

                        
                        <!-- 3º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>3</b> <span class="team"></span> <span>BELATRIX TOW</span></td>
                    <!-- Vitorias --> <td>2</td>
                    <!-- Estrelas --> <td>8</td>
                    <!-- Porcentagem--><td>86,25%</td>
                        </tr>

                        <!-- 4º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>4</b> <span class="team"></span> <span>JAPA TIGERS</span></td>
                    <!-- Vitorias --> <td>1</td>
                    <!-- Estrelas --> <td>7</td>
                    <!-- Porcentagem--><td>81.50%</td>
                        </tr>

                    <!-- 5º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>5</b> <span class="team"></span> <span>ERTY</span></td>
                    <!-- Vitorias --> <td>1</td>
                    <!-- Estrelas --> <td>5</td>
                    <!-- Porcentagem--><td>53,25%</td>
                    </tr>

                    <!-- 6º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>6</b> <span class="team"></span> <span>SPARTTA</span></td>
                    <!-- Vitorias --> <td>-</td>
                    <!-- Estrelas --> <td>-</td>
                    <!-- Porcentagem--><td>-</td>
                    </tr>

                    <!-- 7º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>7</b> <span class="team"></span> <span>PRESIDENTE®</span></td>
                    <!-- Vitorias --> <td>-</td>
                    <!-- Estrelas --> <td>-</td>
                    <!-- Porcentagem--><td>-</td>
                    </tr>

                    <!-- 8º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>8</b> <span class="team"></span> <span>DANILLO-SVS</span></td>
                    <!-- Vitorias --> <td>-</td>
                    <!-- Estrelas --> <td>-</td>
                    <!-- Porcentagem--><td>-</td>
                    </tr>


                    </table>
                </div>
            </div>
            
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Primeira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>YONDAIME &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                        <span>100% : 83%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i>&emsp;ERTY </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;- </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>-&emsp;</span>
                                        </span>
                                        <span class="score">
                                                <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;- </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Segunda rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;- </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;-</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>BELATRIX TOW &emsp; <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>87% : 67%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i>&emsp;JAPA TIGERS </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Terceira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>ERTY &emsp; <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>59% : 79%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;JAPA TIGERS</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;-</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                     </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>JOAOZIN &emsp; <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>91% : 75%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;BELATRIX TOW </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quarta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;-</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>ERTY &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>78% : 72%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i>&emsp;JOAOZIN</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quinta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>YONDAIME &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>98% : 92%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;JAPA TIGERS</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp; </span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;-</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;- </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sexta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span> &emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>- &emsp; </span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;-</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>JAPA TIGERS &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>88% : 100%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;JOAOZIN</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>YONDAIME &emsp;<i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 83%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i>&emsp;BELATRIX TOW</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sétima rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>BELATRIX TOW &emsp;<i class="fa fa-star star1"></i>  <i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 76%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i> &emsp;ERTY</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>YONDAIME &emsp; <i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>58% : 80%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i> <i class="fa fa-star star1"></i>&emsp;JOAOZIN</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;-</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>- &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;-</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

            </div>
    </section>

                </div>

 <!--2 Divisão Grupo A --------------->
<div id="2divisaoa" class="tab-pane fade">
            <table>
                    <tr>
                    <th colspan="2" width="20%" style=" background-color:#44bd32; color:white">Classificado para a fase playoff</th>
                    </tr>
             </table>
    <section class="main-match-shedule">
            <div class="row">
            <div class="col-md-6">
                <h6>Ranking</h6>
                <div class="club-standings overflow-scroll">
                    <table class="table-standings">
                        <tr>
                            <th>Jogador</th>
                            <th>Vitória</th>
                            <th>Estrela</th>
                            <th>Porcentagem</th>
                        </tr>

                        <!-- 1º Lugar-->
                        <tr class="teste">
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;"><b>1</b> <span class="team"></span> <span>JACO MITO</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">5</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">14</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">88,33%</td>
                        </tr>

                        <!-- 2º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>2</b> <span class="team"></span> <span>PATOLINO</span></td>
                    <!-- Vitorias --> <td>4</td>
                    <!-- Estrelas --> <td>13</td>
                    <!-- Porcentagem--><td>88,33%</td>
                        </tr>

                        
                        <!-- 3º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>3</b> <span class="team"></span> <span>DANILO</span></td>
                    <!-- Vitorias --> <td>13</td>
                    <!-- Estrelas --> <td>15</td>
                    <!-- Porcentagem--><td>92,67%</td>
                        </tr>

                        <!-- 4º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>4</b> <span class="team"></span> <span>HEIMDALL SAMA</span></td>
                    <!-- Vitorias --> <td>3</td>
                    <!-- Estrelas --> <td>14</td>
                    <!-- Porcentagem--><td>93,33</td>
                        </tr>

                    <!-- 5º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>5</b> <span class="team"></span> <span>RAFAEL</span></td>
                    <!-- Vitorias --> <td>3</td>
                    <!-- Estrelas --> <td>13</td>
                    <!-- Porcentagem--><td>80%</td>
                    </tr>

                    <!-- 6º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>6</b> <span class="team"></span> <span>MAYHEM</span></td>
                    <!-- Vitorias --> <td>2</td>
                    <!-- Estrelas --> <td>13</td>
                    <!-- Porcentagem--><td>78,33%</td>
                    </tr>

                    <!-- 7º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>7</b> <span class="team"></span> <span> @DALEPRANI</span></td>
                    <!-- Vitorias --> <td>1</td>
                    <!-- Estrelas --> <td>9</td>
                    <!-- Porcentagem--><td>65%</td>
                    </tr>

                    <!-- 8º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>8</b> <span class="team"></span> <span>STRONG_GAB</span></td>
                    <!-- Vitorias --> <td>-</td>
                    <!-- Estrelas --> <td>-</td>
                    <!-- Porcentagem--><td>-</td>
                    </tr>


                    </table>
                </div>
            </div>
            
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Primeira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>HEIMDALL SAMA &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                        <span>95% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DANILO </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>@DALEPRANI&emsp;<i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>61% : 80%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;PATOLINO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>JACO MITO&emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                                <span>77% : 82%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;MAYHEM</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>STRONG_GAB&emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;RAFAEL</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Segunda rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>DANILO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>83% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;MAYHEM </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>@DALEPRANI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>75% : 85%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;RAFAEL</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>HEIMDALL SAMA &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 99%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;PATOLINO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>JACO MITO &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;STRONG_GAB </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Terceira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>PATOLINO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>82% : 68%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;RAFAEL</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>DANILO &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;STRONG_GAB</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>HEIMDALL SAMA &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>99% : 95%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;MAYHEM</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                     </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>@DALEPRANI &emsp;<i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>50% : 81%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;JACO MITO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quarta rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>PATOLINO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>93% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;JACO MITO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>HEIMDALL SAMA &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>80% : 94%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;RAFAEL</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>MAYHEM &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;STRONG_GAB</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>DANILO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 62%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;@DALEPRANI</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quinta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>HEIMDALL SAMA &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;STRONG_GAB</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>RAFAEL &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>77% : 84%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;JACO MITO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>MAYHEM &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i> </span>
                                    </span>
                                    <span class="score">
                                            <span>62% : 67%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;@DALEPRANI</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>PATOLINO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>79% : 74%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DANILO </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sexta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>MAYHEM &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>62% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span> <i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;PATOLINO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>RAFAEL &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i> </span>
                                        </span>
                                        <span class="score">
                                             <span>83% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DANILO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>STRONG_GAB &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;@DALEPRANI</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>HEIMDALL SAMA &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>86% : 88%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;JACO MITO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sétima rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>JACO MITO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 99%</span>
                                        </span>
                                        <span class="team1">
                                            <span> <i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DANILO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>HEIMDALL SAMA &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 75%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i>&emsp; @DALEPRANI</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>STRONG_GAB &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;PATOLINO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>RAFAEL &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>73% : 69%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;MAYHEM</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

            </div>
    </section>

                </div>

<!--2 Divisão Grupo B --------------->
<div id="2divisaob" class="tab-pane fade in">
    <table>
            <tr>
            <th colspan="2" width="20%" style=" background-color:#44bd32; color:white">Classificado para a fase playoff</th>
            </tr>
        </table>
    <section class="main-match-shedule">
            <div class="row">
            <div class="col-md-6">
                <h6>Ranking</h6>
                <div class="club-standings overflow-scroll">
                    <table class="table-standings">
                        <tr>
                            <th>Jogador</th>
                            <th>Vitória</th>
                            <th>Estrela</th>
                            <th>Porcentagem</th>
                        </tr>

                    <!-- 1º Lugar-->
                        <tr class="teste">
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;"><b>1</b> <span class="team"></span> <span>CELINHO</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">7</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">16</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">91%</td>
                        </tr>

                        <!-- 2º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>2</b> <span class="team"></span> <span>TOBIRAMA-CLUB</span></td>
                    <!-- Vitorias --> <td>5</td>
                    <!-- Estrelas --> <td>17</td>
                    <!-- Porcentagem--><td>93,71%</td>
                        </tr>

                        
                        <!-- 3º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>3</b> <span class="team"></span> <span>KAKASHI HATAKI</span></td>
                    <!-- Vitorias --> <td>4</td>
                    <!-- Estrelas --> <td>18</td>
                    <!-- Porcentagem--><td>89,43%</td>
                        </tr>

                        <!-- 4º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td><b>4</b> <span class="team"></span> <span>RIVERA.CR☆☆☆</span></td>
                    <!-- Vitorias --> <td>4</td>
                    <!-- Estrelas --> <td>15</td>
                    <!-- Porcentagem--><td>88,71%</td>
                        </tr>

                    <!-- 5º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>5</b> <span class="team"></span> <span>WILMER</span></td>
                    <!-- Vitorias --> <td>2</td>
                    <!-- Estrelas --> <td>16</td>
                    <!-- Porcentagem--><td>86,57%</td>
                    </tr>

                    <!-- 6º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>6</b> <span class="team"></span> <span>YONDAIME</span></td>
                    <!-- Vitorias --> <td>2</td>
                    <!-- Estrelas --> <td>14</td>
                    <!-- Porcentagem--><td>76,71%</td>
                    </tr>

                    <!-- 7º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>7</b> <span class="team"></span> <span>KÖVEIRO</span></td>
                    <!-- Vitorias --> <td>3</td>
                    <!-- Estrelas --> <td>12</td>
                    <!-- Porcentagem--><td>85%</td>
                    </tr>

                    <!-- 8º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>8</b> <span class="team"></span> <span>DUDUDU</span></td>
                    <!-- Vitorias --> <td>1</td>
                    <!-- Estrelas --> <td>15</td>
                    <!-- Porcentagem--><td>69,43%</td>
                    </tr>

                    <!-- 9º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>9</b> <span class="team"></span> <span>RONNY TAVARES</span></td>
                    <!-- Vitorias --> <td>-</td>
                    <!-- Estrelas --> <td>-</td>
                    <!-- Porcentagem--><td>-</td>
                    </tr>

                    </table>
                </div>
            </div>
            
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Primeira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>TOBIRAMA-CLUB &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                        <span>98% : 47%</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;YONDAIME </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>WILMER&emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>74% : 94%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;CELINHO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>RIVERA.CR☆☆☆&emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                                <span>99% :66%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i>&emsp;DUDUDU</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>RONNY TAVARES&emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;KÖVEIRO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Segunda rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 67%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;YONDAIME </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>TOBIRAMA-CLUB &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 55%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DUDUDU</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>WILMER &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 55%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;KÖVEIRO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>RIVERA.CR☆☆☆ &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;RONNY TAVARES </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Terceira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>74% : 92%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;CELINHO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>YONDAIME &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 71%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DUDUDU</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>TOBIRAMA-CLUB &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;RONNY TAVARES</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                     </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>WILMER &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>89% : 100%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;RIVERA.CR☆☆☆</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quarta rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 72%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DUDUDU</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>CELINHO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>91% : 66%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;KÖVEIRO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>YONDAIME &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;RONNY TAVARES</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>TOBIRAMA-CLUB &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>93% : 100%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;WILMER</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quinta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>69% : 85%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;KÖVEIRO</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>DUDUDU &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;RONNY TAVARES</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>CELINHO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i> </span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 83%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;RIVERA.CR☆☆☆</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>YONDAIME &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 83%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;WILMER </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sexta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span> &emsp;RONNY TAVARES</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KÖVEIRO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i> </span>
                                        </span>
                                        <span class="score">
                                             <span>76% : 98%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;RIVERA.CR☆☆☆</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>DUDUDU &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>81% : 69%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;WILMER</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>CELINHO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>80% : 72%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;TOBIRAMA-CLUB</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sétima rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 80%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i> &emsp;RIVERA.CR☆☆☆</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>RONNY TAVARES &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp; WILMER</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>KÖVEIRO &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>92% : 100%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;TOBIRAMA-CLUB</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>CELINHO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>80% : 73%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;YONDAIME</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Oitava rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 91%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i> &emsp;WILMER</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>RIVERA.CR☆☆☆ &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>89% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp; TOBIRAMA-CLUB</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>KÖVEIRO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>84% : 79%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;YONDAIME</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>DUDUDU &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : -</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;CELINHO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Nona rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>KAKASHI HATAKI &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>83% : 93%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i> &emsp;TOBIRAMA-CLUB</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>RIVERA.CR☆☆☆ &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>72% : 71%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp; YONDAIME</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>RONNY TAVARES &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;CELINHO</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>KÖVEIRO &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 60%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;DUDUDU</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

            </div>
    </section>

                </div>

                 <!--1 Divisão --------------->
<div id="1divisao" class="tab-pane fade in">
            <table>
                    <tr>
                    <th colspan="2" width="20%" style=" background-color:#44bd32; color:white">Classificado para a fase playoff</th>
                    </tr>
             </table>
    <section class="main-match-shedule">
            <div class="row">
            <div class="col-md-6">
                <h6>Ranking</h6>
                <div class="club-standings overflow-scroll">
                    <table class="table-standings">
                        <tr>
                            <th>Jogador</th>
                            <th>Vitória</th>
                            <th>Estrela</th>
                            <th>Porcentagem</th>
                        </tr>

                        <!-- 1º Lugar-->
                        <tr class="teste">
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;"><b>1</b> <span class="team"></span> <span>ALDEANO8</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">5</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">15</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">95,17%</td>
                        </tr>

                        <!-- 2º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;"><b>2</b> <span class="team"></span> <span>EFEX</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">4</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">15</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">82,87%</td>
                        </tr>

                        
                        <!-- 3º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;"><b>3</b> <span class="team"></span> <span>☣EDUDEBOCHADO☣</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">4</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">13</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">87,67%</td>
                        </tr>

                        <!-- 4º Lugar-->
                        <tr>
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;" ><b>4</b> <span class="team"></span> <span>MEIANOITE</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">3</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">14</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">82,67%</td>
                        </tr>

                    <!-- 5º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;"><b>5</b> <span class="team"></span> <span>女神プリシリア❤</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">2</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">13</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">85,33%</td>
                    </tr>

                    <!-- 6º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td style="background-color: #44bd32 !important;"><b>6</b> <span class="team"></span> <span>SR. MALDINII</span></td>
                    <!-- Vitorias --> <td style="background-color: #44bd32 !important;">2</td>
                    <!-- Estrelas --> <td style="background-color: #44bd32 !important;">13</td>
                    <!-- Porcentagem--><td style="background-color: #44bd32 !important;">83%%</td>
                    </tr>

                    <!-- 7º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>7</b> <span class="team"></span> <span> EDSON</span></td>
                    <!-- Vitorias --> <td>1</td>
                    <!-- Estrelas --> <td>13</td>
                    <!-- Porcentagem--><td>78,17%</td>
                    </tr>

                    <!-- 8º Lugar-->
                    <tr>
                    <!-- Nick Vila --><td><b>8</b> <span class="team"></span> <span>BERNAUL</span></td>
                    <!-- Vitorias --> <td>-</td>
                    <!-- Estrelas --> <td>-</td>
                    <!-- Porcentagem--><td>-</td>
                    </tr>


                    </table>
                </div>
            </div>
            
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Primeira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>EDSON &emsp;<i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                        <span>88% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;☣EDUDEBOCHADO☣ </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>女神プリシリア❤&emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 62%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;SR. MALDINII</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>MEIANOITE&emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                                <span>93% : 61%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;EFEX</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>ALDEANO8&emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;BERNAUL</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Segunda rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>☣EDUDEBOCHADO☣ &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>89% : 69%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;EFEX </span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>女神プリシリア❤ &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;BERNAUL</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>EDSON &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>92% : 92%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;SR. MALDINII</span>
                                    </span>
                            </span>
                            <span class="game-result">(3:18' - 3:13')</span>
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>MEIANOITE &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>79% : 100%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;ALDEANO8 </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Terceira rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>SR. MALDINII &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;BERNAUL</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>☣EDUDEBOCHADO☣ &emsp;<i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>62% : 95%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;ALDEANO8</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>EDSON &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>62% : 66%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;EFEX</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                     </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>女神プリシリア❤ &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>80%: 100%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;MEIANOITE</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quarta rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>SR. MALDINII &emsp;</i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>95% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i>&emsp;MEIANOITE</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>EDSON &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;BERNAUL</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>EFEX &emsp;</i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 89%</span>
                                    </span>
                                    <span class="team1">
                                        <span></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i>&emsp;ALDEANO8</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>☣EDUDEBOCHADO☣ &emsp;</i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 72%</span>
                                    </span>
                                    <span class="team1">
                                        <span></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i>&emsp;女神プリシリア❤</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Quinta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>EDSON &emsp;</i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>62% : 100%</span>
                                        </span>
                                        <span class="team1">
                                            <span></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i>&emsp;ALDEANO8</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>BERNAUL &emsp;</span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;MEIANOITE</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>EFEX &emsp;<i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i> </span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 94%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i>&emsp;女神プリシリア❤</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>SR. MALDINII &emsp;<i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 84%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i></i><i class="fa fa-star star1"></i></i>&emsp;☣EDUDEBOCHADO☣ </span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sexta rodada</h6>
                    
                    <!--Partida -->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>EFEX &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>100% : 75%</span>
                                        </span>
                                        <span class="team1">
                                            <span> <i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;SR. MALDINII</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>BERNAUL &emsp; </span>
                                        </span>
                                        <span class="score">
                                             <span>- : -</span>
                                        </span>
                                        <span class="team1">
                                            <span>&emsp;☣EDUDEBOCHADO☣</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>ALDEANO8 &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>87% : 77%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;女神プリシリア❤</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>EDSON &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>85% : 68%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;MEIANOITE</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

                <div class="col-md-6 col-sm-12 col-xs-12">
                    <h6>Sétima rodada</h6>
                    
                    <!--Partida 1-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>MEIANOITE &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>56% : 91%</span>
                                        </span>
                                        <span class="team1">
                                            <span> <i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;☣EDUDEBOCHADO☣</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                        <!--Partida 2-->
                        <div class="main-lates-matches">
                            <a href="#" class="item">
                                <span class="teams-wrap">
                                    <span class="team">
                                            <span>EDSON &emsp;<i class="fa fa-star star1"></i></span>
                                        </span>
                                        <span class="score">
                                             <span>77% : 89%</span>
                                        </span>
                                        <span class="team1">
                                            <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp; 女神プリシリア❤</span>
                                        </span>
                                </span>
                                <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                            </a>
                        </div>

                      <!--Partida 3-->
                      <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>ALDEANO8 &emsp;<i class="fa fa-star star1"></i><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i></span>
                                    </span>
                                    <span class="score">
                                            <span>100% : 74%</span>
                                    </span>
                                    <span class="team1">
                                        <span><i class="fa fa-star star1"></i><i class="fa fa-star star1"></i>&emsp;SR. MALDINII</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                    
                    <!--Partida 4-->
                    <div class="main-lates-matches">
                        <a href="#" class="item">
                            <span class="teams-wrap">
                                <span class="team">
                                        <span>BERNAUL &emsp;</span>
                                    </span>
                                    <span class="score">
                                            <span>- : -</span>
                                    </span>
                                    <span class="team1">
                                        <span>&emsp;EFEX</span>
                                    </span>
                            </span>
                            <!-- <span class="game-result">(2:45' - 3:00') Tempo</span> -->
                        </a>
                    </div>

                </div>

            </div>
    </section>
</div>

                <? include "ranking.php"?>
</div>



         </div>
        </div>

    </div>
</section>
